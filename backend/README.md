...
Create a `.env` file in `backend/` if you need to override the defaults.

### Development
```bash
# from repo root
npm run dev:backend
# or, in watch-mode directly inside backend
...
### Deploy
Host the compiled `dist/` folder (e.g. Render, Railway, Fly).  Be sure to set `JWT_SECRET` and allow CORS for your Vercel domain:
```ts
